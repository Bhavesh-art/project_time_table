...
# Year-Semester to batch mapping
year_mapping = {
...
class TimetableApp:
    def __init__(self, root):
        ...
        # Styling
        style = ttk.Style()
        style.configure("TButton", font=("Times New Roman", 14, "bold"), padding=10)
        style.configure("TLabel", font=("Georgia", 14, "bold"))

        # File selection
        ttk.Label(root, text="Select Timetable File:", style="TLabel").grid(row=0, column=0, padx=10, pady=10)
        ttk.Button(root, text="Browse File", command=self.browse_file).grid(row=0, column=1, padx=10, pady=10)

        # Year selection
        ttk.Label(root, text="Select Year/Semester:", style="TLabel").grid(row=1, column=0, padx=10, pady=10)
        ...
        # Batch selection
        ttk.Label(root, text="Select Batch:", style="TLabel").grid(row=2, column=0, padx=10, pady=10)
        ...
        ttk.Button(root, text="Show Timetable", style="TButton", command=self.show_timetable).grid(
    ...
    def load_batches(self, event):
        """Load batches based on the selected year/semester."""
    ...
    def show_timetable(self):
        ...
        try:
            ...
            # Add a title at the top of the timetable window
            ttk.Label(
                timetable_window,
            ...
            font = ("Arial", 10)
            max_widths = [get_text_width(slot, font) for slot in ["DAY"] + time_slot]

            for row in timetable_data:
                for col_index, cell in enumerate(row):
                    cell_width = get_text_width(cell, font)
                    if cell_width > max_widths[col_index + 1]:
                        max_widths[col_index + 1] = cell_width
